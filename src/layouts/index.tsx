import React, { useState, ReactHTMLElement, ReactNode } from 'react';
...
export default (props: any) => {
  ...
  return (
    <Layout style={{ height: '100vh' }}>
      ...
      <Layout>
        <Header style={{ background: '#fff', padding: '0px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <>
            {React.createElement(collapsed ? MenuUnfoldOutlined : MenuFoldOutlined, {
              className: 'trigger',
              onClick: () => setCollapsed(!collapsed)
            })}
            <span style={{ marginLeft: '20px' }} onClick={() => setText(text + '!')}>{text}</span>
          </>
